...
class PaymentSettings extends Page implements HasSchemas
{
        ...
        return $schema
            ->schema([
                ...
                Section::make('General')
                    ->schema([
                        Select::make('default_gateway')
                            ...
                            ->helperText('The preferred gateway when both are available. If only one is enabled, it will be used regardless of this setting.'),
                    ]),
                ...
                Section::make('Stripe Live Keys')
                    ...
                    ->collapsible()
                    ->collapsed()
                    ->description('⚠️ Live keys for production use only. Secrets are encrypted at rest.'),
                ...
                Section::make('Vipps Configuration')
                    ->schema([
                        ...
                        Toggle::make('vipps_use_sandbox')
                            ->label('Use Sandbox')
                            ->helperText('Toggle between test and live credentials')
                            ->live(),
                            
                        TextInput::make('vipps_callback_prefix')
                            ->label('Callback URL Prefix')
                            ->placeholder('https://your-domain.tld/payments/vipps')
                            ->maxLength(2048)
                            ->url()
                            ->helperText('Base URL for Vipps payment callbacks'),
                    ])
                    ->collapsible()
                ...
                Section::make('Vipps Test Credentials')
                    ...
                    ->collapsible()
                    ->collapsed()
                    ->description('Test credentials for Vipps sandbox environment.'),
                ...
                Section::make('Vipps Live Credentials')
                    ...
                    ->collapsible()
                    ->collapsed()
                    ->description('⚠️ Live credentials for production use only. Secrets are encrypted at rest.'),
                ])
                ->statePath('data')
    ...
    protected function validateGatewaySettings(array $data): void
    {
        ...
        if (!empty($data['default_gateway'])) {
            $gatewayEnabled = $data[$data['default_gateway'] . '_enabled'] ?? false;
            if (!$gatewayEnabled) {
                $this->addError('default_gateway', 'The selected default gateway must be enabled.');
                throw new Halt();
            }
        }
        ...
        if ($data['stripe_enabled'] ?? false) {
            ...
            if ($useTestMode) {
                if (empty($data['stripe_test_publishable_key'])) {
                    $this->addError('stripe_test_publishable_key', 'Test publishable key is required when Stripe is enabled in test mode.');
                    throw new Halt();
                }
                if (empty($data['stripe_test_secret_key'])) {
                    $this->addError('stripe_test_secret_key', 'Test secret key is required when Stripe is enabled in test mode.');
                    throw new Halt();
                }
            } else {
                if (empty($data['stripe_live_publishable_key'])) {
                    $this->addError('stripe_live_publishable_key', 'Live publishable key is required when Stripe is enabled in live mode.');
                    throw new Halt();
                }
                if (empty($data['stripe_live_secret_key'])) {
                    $this->addError('stripe_live_secret_key', 'Live secret key is required when Stripe is enabled in live mode.');
                    throw new Halt();
                }
            }
        }
        ...
        if ($data['vipps_enabled'] ?? false) {
            ...
            if ($useSandbox) {
                $requiredVippsFields = [
                    'vipps_test_merchant_serial_number' => 'Test Merchant Serial Number',
                ...
                $requiredVippsFields = [
                    'vipps_live_merchant_serial_number' => 'Live Merchant Serial Number',
            ...
            foreach ($requiredVippsFields as $field => $label) {
                if (empty($data[$field])) {
                    $this->addError($field, "{$label} is required when Vipps is enabled.");
                    throw new Halt();
                }
            }
        }
